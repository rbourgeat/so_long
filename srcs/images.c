...
/*   Updated: 2021/08/15 21:22:54 by rbourgea         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	const double pixe_x = ((double)(100 * vars->map_x_len) / vars->map_x_len);
	const double pixe_y = ((double)(100 * vars->map_y_len) / vars->map_y_len);
	char *addr;
	double x;
	double y = 0.0;
	...
	while (y < pixe_y)
	{
		...
		while (x < pixe_x)
		{
			...
			pixel_put_color(vars, x + (x_index * pixe_x), y + (y_index * pixe_y), *(unsigned int *)addr);
			x++;
		}
		y++;
	}
